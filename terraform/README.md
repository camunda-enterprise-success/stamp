...
> ⚠️ All `terraform` commands must be run from within the `terraform` subdirectory of your chosen infrastructure type, not from the repo root.
...
> ⚠️ **The state storage must exist before running `terraform init`** — if it doesn't, the backend configuration will fail.

> ⚠️ **Never delete the state storage** while infrastructure is running. Terraform will lose track of what it created, making it very difficult to manage or destroy resources.
...
> ⚠️ This is irreversible. All cloud resources will be permanently deleted. Make sure you have backups of any important data before running this.
